...
local function avg_ms(label, fn)
  for _ = 1, WARMUP do fn() end
  local t0 = os.clock()
  for _ = 1, ITERS do fn() end
  local ms = (os.clock() - t0) * 1000 / ITERS
  print(string.format("RESULT|lua|%s|%.6f", label, ms))
  return ms
end
...
describe("Performance", function()
  ...
  it("encode length 6 averages <= 0.1 ms", function()
    local n = 0
    local ms = avg_ms("encode/6", function()
      n = (n + 1) % odoid.MAX[6]
      odoid.encode(n, 6)
    end)
  ...
  it("encode length 7 averages <= 0.1 ms", function()
    local n = 0
    local ms = avg_ms("encode/7", function()
      n = (n + 1) % odoid.MAX[7]
      odoid.encode(n, 7)
    end)
    assert.is_true(ms <= LIMIT_MS,
  ...
  it("encode length 8 averages <= 0.1 ms", function()
    local n = 0
    local ms = avg_ms("encode/8", function()
      n = (n + 1) % odoid.MAX[8]
      odoid.encode(n, 8)
    end)
    assert.is_true(ms <= LIMIT_MS,
  ...
  it("decode length 6 averages <= 0.1 ms", function()
    ...
    local i = 0
    local ms = avg_ms("decode/6", function()
      i = i + 1
  ...
  it("decode length 7 averages <= 0.1 ms", function()
    ...
    local i = 0
    local ms = avg_ms("decode/7", function()
      i = i + 1
      odoid.decode(ids[(i % #ids) + 1])
    end)
    assert.is_true(ms <= LIMIT_MS,
  ...
  it("decode length 8 averages <= 0.1 ms", function()
    ...
    local i = 0
    local ms = avg_ms("decode/8", function()
      i = i + 1
      odoid.decode(ids[(i % #ids) + 1])
    end)
    assert.is_true(ms <= LIMIT_MS,
  ...
  it("generator:next() averages <= 0.1 ms", function()
    local gen = odoid.generator.new({length = 6})
    local ms = avg_ms("generate/6", function()
      gen:next()
    end)
    assert.is_true(ms <= LIMIT_MS,
      string.format("generate/6: %.4f ms exceeded %.4f ms", ms, LIMIT_MS))
  end)
